...
export default function LoginForm() {
  ...
  return <form className="evaluator-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setError(""); startTransition(async () => { try { const result = await evaluatorLogin({ email: String(form.get("email") ?? ""), password: String(form.get("password") ?? "") }); if (result.ok) { toast.success("Signed in successfully."); router.push("/evaluator"); } else { toast.error("Sign-in failed. Check your credentials."); setError(result.error ?? "Unable to sign in."); } } catch { toast.error("Sign-in failed. Please try again."); setError("Unable to sign in. Please try again."); } }); }}>
