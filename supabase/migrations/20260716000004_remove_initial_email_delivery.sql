-- Stop inserting initial email_delivery rows during submission.
-- The applicant sees their reference/token on screen immediately;
-- no email is needed until a decision is made.
-- The email_deliveries kind constraint still allows 'initial' for any
-- orphan rows left by prior deployments.
