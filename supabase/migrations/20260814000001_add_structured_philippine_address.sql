-- Store each address part independently while retaining the formatted receipt address.
...
drop function if exists public.create_public_submission(uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,date,text,timestamptz,timestamptz,text,text,text,text,integer,integer);

create function public.create_public_submission(
  ...
  p_gender text, p_category text, p_philippine_address text, p_address_line_1 text, p_address_line_2 text, p_barangay text, p_municipality text, p_postal_code text, p_province text, p_region text,
  p_position text, p_jobsite text, p_contact_number text, p_departure_date date, p_details text, p_issued_at timestamptz, p_expires_at timestamptz, p_receipt_token_hash text,
...
begin
  ...
  if p_request_id is null or p_first_name is null or char_length(trim(p_first_name)) not between 1 and 80 or char_length(coalesce(p_middle_name,'')) > 80
    ...
    or p_address_line_1 is null or char_length(trim(p_address_line_1)) not between 1 and 150 or char_length(coalesce(p_address_line_2,'')) > 150
    or p_barangay is null or char_length(trim(p_barangay)) not between 1 and 100 or p_municipality is null or char_length(trim(p_municipality)) not between 1 and 100
    or p_postal_code is null or p_postal_code !~ '^[0-9]{4}$' or p_province is null or char_length(trim(p_province)) not between 1 and 100 or p_region is null or char_length(trim(p_region)) not between 1 and 100
...
revoke all on function public.create_public_submission(uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,date,text,timestamptz,timestamptz,text,text,text,text,integer,integer) from public, anon, authenticated;
grant execute on function public.create_public_submission(uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,date,text,timestamptz,timestamptz,text,text,text,text,integer,integer) to service_role;
