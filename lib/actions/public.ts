...
export async function createSubmission(input: PublicApplicationInput, ipKeyHash: string, emailKeyHash: string) {
  ...
  const philippineAddress = [value.addressLine1, value.addressLine2, `Barangay ${value.barangay}`, value.municipality, value.province, value.region, value.postalCode, "Philippines"].filter(Boolean).join(", ");
  const { data, error } = await createAdminClient().rpc("create_public_submission", { p_request_id: value.requestId, p_first_name: value.firstName, p_middle_name: value.middleName, p_last_name: value.lastName, p_suffix: value.suffix, p_full_name: fullName, p_email: value.email, p_oec_number: value.oecNumber, p_gender: value.gender, p_category: value.category, p_philippine_address: philippineAddress, p_address_line_1: value.addressLine1, p_address_line_2: value.addressLine2, p_barangay: value.barangay, p_municipality: value.municipality, p_postal_code: value.postalCode, p_province: value.province, p_region: value.region, p_position: value.position, p_jobsite: value.jobsite, p_contact_number: value.contactNumber, p_departure_date: new Date().toISOString().slice(0, 10), p_details: "", p_issued_at: issuedAt.toISOString(), p_expires_at: expiresAt.toISOString(), p_receipt_token_hash: hashReceiptToken(token), p_receipt_token_ciphertext: encryptReceiptToken(token), p_ip_key_hash: ipKeyHash, p_email_key_hash: emailKeyHash, p_max_attempts: e.RATE_LIMIT_MAX_ATTEMPTS, p_window_minutes: e.RATE_LIMIT_WINDOW_MINUTES }).single();
...
export async function retrieveReceipt(token: unknown): Promise<PublicReceipt> {
  ...
  const { data, error } = await createAdminClient().from("submissions").select(
    "reference,oec_number,status,issued_at,expires_at,full_name,first_name,middle_name,last_name,suffix,email,gender,category,philippine_address,address_line_1,address_line_2,barangay,municipality,postal_code,province,region,position,jobsite,contact_number,departure_date,details,decision_reason,decided_at,created_at"
  ...
  return {
    ...
    philippineAddress: data.philippine_address, addressLine1: data.address_line_1, addressLine2: data.address_line_2, barangay: data.barangay, municipality: data.municipality, postalCode: data.postal_code, province: data.province, region: data.region,
