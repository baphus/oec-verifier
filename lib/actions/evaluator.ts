...
export async function discardSubmission(id: string, reason?: string) { const { user } = await requireActiveEvaluator(); const parsed = decisionSchema.parse({ id, reason }); const { data, error } = await createAdminClient().rpc("transition_submission", { p_submission_id: parsed.id, p_status: "discarded", p_actor: user.id, p_reason: parsed.reason ?? null }); if (error || !data) throw new Error("This submission could not be discarded. It may already have a decision."); return { ok: true as const }; }
