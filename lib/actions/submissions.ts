...
export async function submitPublicApplication(form: FormData) {
  ...
  const parsed = publicApplicationSchema.safeParse({ firstName: get("firstName"), middleName: get("middleName"), lastName: get("lastName"), suffix: get("suffix"), fullName: get("fullName"), email: get("email"), oecNumber: get("oecNumber"), gender: get("gender"), category: get("category") || get("role"), philippineAddress: get("philippineAddress") || get("address"), province: get("province"), region: get("region"), employer: get("employer"), position: get("position"), jobsite: get("jobsite") || get("airport"), contactNumber: get("contactNumber") || get("phone"), departureDate: get("departureDate"), airport: get("airport"), role: get("role"), phone: get("phone"), details: get("details"), requestId: get("requestId"), consent: form.get("consent") });
  ...
  try { emailWarning = !(await deliverPendingEmail(result.id, "initial")).sent; } catch { emailWarning = true; }
  return { referenceNumber: result.reference, token: result.token, emailWarning, deliveryOutcome: emailWarning ? "queued_for_retry" as const : "sent" as const };
}
