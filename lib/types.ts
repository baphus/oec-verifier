import { z } from "zod";
import { provincesByRegion, regions } from "./data/philippines";
...
export const publicApplicationSchema = z.object({
  ...
  gender: text(40), category: text(100), philippineAddress: text(300),
  addressLine1: text(150), addressLine2: z.string().trim().max(150).optional().default(""), barangay: text(100), municipality: text(100), postalCode: z.string().trim().regex(/^\d{4}$/, "Enter a 4-digit Philippine postal code."),
  province: text(100), region: text(100),
  ...
  if (!regions.includes(value.region as (typeof regions)[number])) {
    ctx.addIssue({ code: "custom", path: ["region"], message: "Select a Philippine region." });
  }
  if (!provincesByRegion[value.region]?.includes(value.province)) {
    ctx.addIssue({ code: "custom", path: ["province"], message: "Select a province within the selected region." });
  }
});
...
export type SubmissionRecord = { id: string; reference: string; first_name: string; middle_name: string; last_name: string; suffix: string; full_name: string; email: string; oec_number: string; gender: string; category: string; philippine_address: string; address_line_1: string | null; address_line_2: string | null; barangay: string | null; municipality: string | null; postal_code: string | null; province: string; region: string; employer: string; position: string; jobsite: string; contact_number: string; departure_date: string; details: string; issued_at: string; expires_at: string; expired_at: string | null; status: SubmissionStatus; decision_reason: string | null; decided_at: string | null; decided_by: string | null; created_at: string };
export type PublicReceipt = {
  reference: string; oecMasked: string;
  ...
  philippineAddress: string; addressLine1: string | null; addressLine2: string | null; barangay: string | null; municipality: string | null; postalCode: string | null; province: string; region: string;
